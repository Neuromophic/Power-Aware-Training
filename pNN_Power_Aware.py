...
class InvRT(torch.nn.Module):
    def __init__(self, args):
        ...
        self.power_estimator = package['power_estimator']
        for name, param in self.power_estimator.named_parameters():
            param.requires_grad = False
        self.power_estimator.train(False)
        self.pow_X_max = package['X_max']
        self.pow_X_min = package['X_min']
        self.pow_Y_max = package['Y_max']
        self.pow_Y_min = package['Y_min']
...
class pLayer(torch.nn.Module):
    def __init__(self, n_in, n_out, args, ACT, INV):
        ...
        theta = torch.rand([n_in + 2, n_out])/1000. + args.gmin
